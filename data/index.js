const emotes = require('./JSON/emotes.json'),
    devs = require('./JSON/dev.json'),
    crime = require('./JSON/crime.json'),
    work = require('./JSON/work.json'),
    beg = require('./JSON/beg.json'),
    core = require('./JSON/core.json'),
    testers = require('./JSON/betaTesters.json'),
    id = require('./JSON/id.json'),
    market = require('./JSON/market.json'),
    staff = require('./JSON/staff.json'),
    kpd = require('./JSON/kpd.json'),
    earlySupporter = require('./JSON/early-supporter.json'),
    xpCommands = require('./JSON/xpcommands.json'),
    bugHunters = require('./JSON/bug-hunters.json');
module.exports = { emotes, crime, work, beg, devs, core, testers, id, market, staff, earlySupporter, kpd, xpCommands, bugHunters };
