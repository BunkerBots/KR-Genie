...
module.exports = {
    ...
    execute: async(message, args) => {
        ...
        if (isNaN(args[0])) return message.reply('What do I look like to you? Provide a valid amount to bet');
        const KRargs = parseInt(args[0]);
        ...
        if (res == 1) {
            ...
            message.reply(new MessageEmbed()
                .setColor('GREEN')
                .setAuthor(message.author.username, message.author.displayAvatarURL({ dynamic: false }))
                .setDescription(`Lucky ducky , you won the bet! ${data.emotes.kr}${args[0]}`)
                .setFooter('stonks4u'));
        } else {
            await db.utils.addKR(message.author.id, -KRargs);
            message.reply(new MessageEmbed()
