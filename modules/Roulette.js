...
class Roulette {
        ...
        this.timeOut = channel.client.setTimeout(async(roulette) => {
            ...
            for (const entity of roulette.players) {
                ...
                const embed = new MessageEmbed()
                    .setTitle('Roulette')
                    .setDescription(winners.size ? parseCollection(winners) : 'No Winners')
    ...
    async addPlayer(user, bet, money) {
        await db.utils.removeKR(user.id, money);
...
const parseCollection = (collection) =>
    'Winners: \n' + collection.map((k, v) => `<@${k}> has won ${emotes.kr} ${v}`).join('\n');
